...
static const char *TAG = "PLAY_WAV_FLASH";
/*
...
// extern const uint8_t adf_music_wav_start[] asm("_binary_sample_02_wav_start");
// extern const uint8_t adf_music_wav_end[]   asm("_binary_sample_02_wav_end");
...
// {
//     static int wav_pos;
//     int read_size = adf_music_wav_end - adf_music_wav_start - wav_pos;
//     if (read_size == 0) {
//         return AEL_IO_DONE;
//     } else if (len < read_size) {
//         read_size = len;
//     }
//     memcpy(buf, adf_music_wav_start + wav_pos, read_size);
//     wav_pos += read_size;
//     return read_size;
// }
...
void app_main(void)
{
    ...
    audio_element_handle_t i2s_stream_writer, mp3_decoder,wav_decoder;
    ...
    ESP_LOGI(TAG, "[2.1.1] Create wav decoder to decode wav file and set custom read callback");
    ...
    const char *link_tag[4] = {"mp3", "wav", "filter", "i2s"};
    audio_pipeline_link(pipeline, &link_tag[0], 4);
#else
    ESP_LOGI(TAG, "The current use board is ESP32-LyraT-Mini");
    const char *link_tag[3] = {"mp3","wav","i2s"};
